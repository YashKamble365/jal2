// src/services/enhancedGeographicService.ts - INTEGRATED WITH YOUR EXISTING SERVICES
import { Coordinates } from '../types/environmental';
import { PropertyDetails, SubsidyInfo } from '../types/recommendations';
import { ALL_STATES_SUBSIDY_DATA, StateSubsidyScheme } from '../config/allStatesSubsidyData';
...
export class EnhancedGeographicService {
  ...
  async getStateFromCoordinates(coordinates: Coordinates): Promise<StateSubsidyScheme | null> {
    try {
      ...
      const matchingState = ALL_STATES_SUBSIDY_DATA.find(state => 
        state.state.toUpperCase() === locationInfo.state.toUpperCase() ||
  ...
  private findClosestStateByCoordinates(coordinates: Coordinates): StateSubsidyScheme {
  ...
  private calculateDistanceToStateBounds(coordinates: Coordinates, state: StateSubsidyScheme): number {
  ...
  async getSubsidyInformation(
    ...
    try {
      ...
      const applicableSchemes = stateData.schemes.filter(scheme => 
        scheme.eligibility.propertyTypes.includes(propertyDetails.propertyType)
      );
      ...
      const bestScheme = applicableSchemes.reduce((best, current) => 
        current.subsidy.maxAmount > best.subsidy.maxAmount ? current : best
      );
      ...
      const calculatedSubsidy = Math.min(
        budget * (bestScheme.subsidy.percentage / 100),
        bestScheme.subsidy.maxAmount
      );
  ...
  private checkEligibility(scheme: any, propertyDetails: PropertyDetails): boolean {
    // Check property type
  ...
  private getDefaultSubsidyInfo(budget: number): SubsidyInfo {
  ...
  async getMarketModifier(coordinates: Coordinates): Promise<number> {
    try {
      const locationInfo = await locationService.getAdministrativeContext(coordinates);
      const stateData = await this.getStateFromCoordinates(coordinates);
  ...
  private getRegionalPriceModifier(state: string, district: string): number {
  ...
  async getAllSchemesForLocation(coordinates: Coordinates) {
    return await this.getStateFromCoordinates(coordinates);
  }
  
  // Search states by name (for manual selection)
  searchStateByName(stateName: string): StateSubsidyScheme | null {
    return ALL_STATES_SUBSIDY_DATA.find(state => 
  ...
  getAllStates(): { name: string, code: string }[] {
